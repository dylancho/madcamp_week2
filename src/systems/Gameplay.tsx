...
class GameplaySys {
    ...
    handleKeyDown = (e: KeyboardEvent) => {
        this.pressedKeys[e.key] = true; // Mark the key as pressed
    
        if (e.key === "2" && this.is3DMode()) {
            this.setIs3DMode(false);
        } else if (e.key === "3" && !this.is3DMode()) {
        ...
        if (this.is3DMode()) {
            ...
            if (this.pressedKeys["ArrowUp"]) this.setVelocity('y', this.speed); // Move up
            if (this.pressedKeys["ArrowDown"]) this.setVelocity('y', -this.speed); // Move down
        } else {
        // Jump in 2D mode
        if (this.pressedKeys["ArrowUp"]) this.jump();
        }
    }
