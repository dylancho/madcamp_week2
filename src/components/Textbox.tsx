...
export const AccountTextbox: Component<{
  ...
  return (
    <input
      ...
      placeholder={message}
      oninput={(e) =>
        dataSys.setCurCreatingAccount(field, e.currentTarget.value)
      }
    />
  );
};
...
export const SearchTextbox: Component<{ message: string }> = ({ message }) => {
  return (
    <div class={`${TextboxStyle(Size.ui.searchW)} ${searchTextboxStyle}`}>
      ...
      <input class={inputStyle} placeholder={message} oninput={(e) => {dataSys.setSearchQuery(e.currentTarget.value)}} />
    </div>
  );
};
